...
def Input1(dt):
    ...
    for i in odleg:
        # Leg 1, 3, 5
        PosY[i] = BodyCenterOffsetY[i] + Offset * sin(radians(FeetAngle[i])) - dt
        PosZ[i] = -Height
...
# Initial Motion (Start Position)
PosX, PosY, PosZ = Initial()
th = IK(PosX, PosY, PosZ)
Generating(th)
time.sleep(1)

# Motion Generating (Steps)
index = 0
step = 0
# dt 값 부호 전환으로 보행 방향 앞&뒤 전환 가능
dt = 25
while 1:

    if index == 0:
        for i in range (-dt, dt):
            PosX, PosY, PosZ = Input1(i*2)
            th = IK(PosX, PosY, PosZ)
            Generating(th)
            time.sleep(0.0001)
